...
router.post('/',Authenticate, async (req, res) => {
  ...
  try {
    ...
    const job = new Job({ jobTitle, jobDescription, experience, salary, company_name: req.rootUser.name, comp_id: req.rootUser._id,profile: req.rootUser.profile});
    await job.save();
...
router.post('/apply',Authenticate, async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Job Applied Successfully', jobApplication });
  } catch (error) {
